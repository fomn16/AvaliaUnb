...
global.app.post('/auth/register', async (req, res, next) => {
    ...
    .then(usuarioExiste =>{
        ...
        .then(usuarioBd => {
            ...
            return res.status(200).json({nome: usuarioBd.nome, email:usuarioBd.email, matricula: usuarioBd.matricula, token:token, avatar:usuarioBd.avatar, administrador: usuarioBd.administrador});
        })
        .catch(err => {
            console.log(err);
    ...
    return RepositorioUsuario.Get(credenciais.matricula).then(usuario => {
        ...
        bcrypt.compare(credenciais.senha + usuario.salt, usuario.senha, (err, result) => {
            ...
            if (result) {
                ...
                return res.status(200).json({nome: usuario.nome, email:usuario.email, matricula: usuario.matricula, token:token, avatar:usuario.avatar, administrador:usuario.administrador});
            }
            return res.status(403).json({error:'matrícula ou senha errada'});
        });        
    })
